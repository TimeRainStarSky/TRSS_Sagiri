...
echo -n "docker exec -it $DKNAME bash /root/TRSS_Sagiri/Main.sh "'"$@"'>"$CMDPATH/$CMD"&&chmod 755 "$CMDPATH/$CMD"||abort "脚本执行命令 $CMDPATH/$CMD 设置失败，手动执行命令：docker exec -it $DKNAME bash /root/TRSS_Sagiri/Main.sh"
echo "
