...
$B  最新版本：$G$NEWNAME$C ($NEWVER)$O
  ...
  开始下载";mkdir -vp "$DIR";geturl "$URL/Main.sh">"$DIR/Main.sh"||abort_update "下载失败";[ "$(md5sum "$DIR/Main.sh"|head -c 32)" != "$MD5" ]&&abort_update "下载文件校验错误";echo -n "bash '$DIR/Main.sh' "'"$@"'>/bin/tssi||abort "脚本执行命令/bin/tssi设置失败";chmod 755 /bin/tssi||abort "脚本权限设置失败";echo "
$G- 脚本下载完成$O";}
echo "
...
COPY Main.sh /root/TRSS_Sagiri/Main.sh
COPY tssi /bin
...
docker run -itd --restart=always --name TRSS_Sagiri trss:sagiri
echo -n "docker exec -it TRSS_Sagiri bash '$DIR/Main.sh' "'"$@"'>/bin/tssi||abort "脚本执行命令/bin/tssi设置失败";chmod 755 /bin/tssi||abort "脚本权限设置失败";echo "
$G- Docker 容器安装完成，输入tssi执行$O";exit;}